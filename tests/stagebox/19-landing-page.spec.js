...
test.describe('Landing page — carregamento', () => {
  ...
  test('título da página está correto', async ({ page }) => {
    ...
    await expect(page).toHaveTitle(/StageBox/i);
  });
...
test.describe('Landing page — navbar', () => {
  test('logo/brand está visível', async ({ page }) => {
    ...
    await expect(brand).toContainText('StageBox');
  });
...
test.describe('Landing page — hero', () => {
  ...
  test('badge "Grátis por 30 dias" está visível', async ({ page }) => {
    await page.goto('/landing.php');
    const badge = page.locator('.hero-badge, [class*="badge"]').first();
    await expect(badge).toBeVisible();
    const text = await badge.textContent();
    expect(text).toMatch(/30 dias|grátis/i);
  });

  test('botão primário "Testar grátis por 30 dias" leva para register.php', async ({ page }) => {
    await page.goto('/landing.php');
    // CTA principal no hero
    const ctaBtn = page.locator('.hero .btn-primary, .cta-group .btn-primary').first();
    await expect(ctaBtn).toBeVisible();
    await expect(ctaBtn).toContainText(/testar grátis/i);
    await ctaBtn.click();
    await page.waitForURL('**/register.php');
    expect(page.url()).toContain('register.php');
  });
  ...
  test('nota de rodapé do hero menciona preço e cancelamento', async ({ page }) => {
    await page.goto('/landing.php');
    const note = page.locator('.hero-note').first();
    await expect(note).toBeVisible();
    const text = await note.textContent();
    expect(text).toMatch(/R\$5|gratu|cancel/i);
  });
});
...
test.describe('Landing page — pricing', () => {
  ...
  test('preço R$5/mês está exibido', async ({ page }) => {
    await page.goto('/landing.php');
    const pricing = page.locator('.pricing');
    await expect(pricing).toContainText(/R\$5/);
  });

  test('lista de benefícios está visível', async ({ page }) => {
    await page.goto('/landing.php');
    const features = page.locator('.price-features li');
  ...
  test('botão "Testar grátis" no pricing leva para register.php', async ({ page }) => {
    await page.goto('/landing.php');
    const pricingBtn = page.locator('.pricing .btn-primary').first();
  ...
  test('preço anual mencionado', async ({ page }) => {
    await page.goto('/landing.php');
    const pricing = page.locator('.pricing');
    await expect(pricing).toContainText(/R\$50|ano/i);
  });
});
...
test.describe('Landing page — footer', () => {
  ...
  test('footer contém nome StageBox', async ({ page }) => {
    await page.goto('/landing.php');
    const footer = page.locator('footer');
    await expect(footer).toContainText('StageBox');
  });
...
test.describe('Landing page — fluxo completo do visitante', () => {
  ...
  test('visitante rola até pricing → clica CTA → vê registro', async ({ page }) => {
    ...
    await page.locator('.pricing .btn-primary').first().click();
    await page.waitForURL('**/register.php');
    expect(page.url()).toContain('register.php');
  });
