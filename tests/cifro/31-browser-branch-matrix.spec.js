...
test('exercita decisões reais de apresentação, tema, confirmação e toast', async ({ page }) => {
  await page.addInitScript(() => {
    Object.defineProperty(navigator, 'wakeLock', {
      configurable: true,
      value: { request: async () => ({ addEventListener() {}, release() {} }) },
    });
    document.documentElement.requestFullscreen = () => Promise.reject(new Error('indisponível'));
  });
  ...
  await page.evaluate(() => {
    ...
    emit('touchend', [{ clientX: 110, clientY: 100 }]);
    emit('touchstart', [{ clientX: 100, clientY: 100 }]);
...
test('live alterna líder, seguidor, desconexão e páginas reais', async ({ page, context }) => {
  ...
  await page.evaluate(({ host, mode }) => {
    ...
    return window.LiveMode.atualizarPaginaHost(false);
  }, keys);
  ...
  await page.evaluate(() => window.LiveMode.assumirHost());
  await expect(page.locator('#liveStatus')).toContainText('host', { timeout: 10000 });
  await page.evaluate(() => Promise.all([window.LiveMode.atualizarPaginaHost(false), window.LiveMode.atualizarPaginaHost(true)]));
  await page.evaluate(mode => {
    sessionStorage.setItem(mode, 'follow');
    window.LiveMode.entrarOuSairLive();
  }, keys.mode);
  await expect(page.locator('#liveStatus')).toContainText('desconectada');

  await openPreview(page);
  await page.evaluate(({ host, mode }) => {
    sessionStorage.setItem(mode, 'host');
    if (!localStorage.getItem(host)) localStorage.setItem(host, 'inexistente');
  }, keys);
  ...
  await page.goto('/music.php?id=inválido&playlistTom=H');
  await page.evaluate(({ host, mode }) => {
    sessionStorage.setItem(mode, 'host');
    localStorage.removeItem(host);
    return window.LiveMode.atualizarPaginaHost(false);
  }, keys);
  await expect(page.getByRole('heading', { name: 'Música não encontrada' })).toBeVisible();
});
